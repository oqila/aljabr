...
int main(int argc, char *argv[])
	...
	float x0=1;
	int n=0,i,ftype=0;				 
	char * filename=NULL, * plugin_func ;
	...
	plugin_func = (char * ) malloc (sizeof(char[10]));
	if( plugin_func==NULL )
		{ ERROR("Can not allocate enough space for variables\n");exit(-1);}
	
	bitmap=cmd_args_support(argc, argv, filename, interval_a, interval_b, n, ftype, plugin_func);
...
splash->showMessage(QObject::tr("Loading..."),Qt::AlignLeft | Qt::AlignBottom, Qt::black);
...
///////////////
	...
	MSG("Processing X&Y data ...\n");
	
	if(bitmap & FJ::Plugin){
if(dll_function(x,f,interval_a,interval_b,n,filename,bitmap,plugin_func,&func[ftype])<0)exit(-1);
				}
	else
	if(FindXY[ftype](x,f,interval_a,interval_b,n,filename,bitmap)<0)exit(-1);
	//printf("ftype = %i\n",ftype);
