...
int cmd_args_support(int argc, char **argv, char *filename, float &a , float &b , int &n,int & type, char * function)
	{
	int bitmap=0;
	...
	OptionPattern  optpattern[]={
					...
					{14, "-d",	"--plugin",	REQUIRE_ARGUMENT}
				      };
	

	optlist=Process_Cmd_Option_List(argc, argv, optpattern,14, opterror);
	...
	while (temp)
		{
			...
			switch(temp->optId)
			{
			case 1: bitmap|= FJ::InputFile; strcpy(filename,temp->argument); type=FJ::FROM_FILE;printf("type = %i\n",type); break;
			...
			case 9: bitmap|= FJ::InputFile; strcpy(filename,temp->argument); type=FJ::FROM_FILE; break;
			case 10: bitmap|= FJ::FuncType; strcpy(function,temp->argument); break;
			case 11: bitmap|= FJ::LinearMethod;break;
			case 12: bitmap|= FJ::ParabolicMethod;break;
			case 14: bitmap|= FJ::Plugin; strcpy(filename,temp->argument);break;
			default :break;
			}
			temp=temp->next; 
		}
	...
	if((bitmap & FJ::Plugin) && !(bitmap & FJ::FuncType))
	{
	ERROR("");
	MSG("-d/--plugin option must be used with -t/--func-type option\n");
	info(FJ::Hint);
	return -1;
	}
	...
	{
		if(!strcmp(function,"cos"))type=FJ::COSX;
		else if	(!strcmp(function,"sin"))type=FJ::SINX;
		else if	(!strcmp(function,"log"))type=FJ::NATURAL_LOGX;
		else if	(!strcmp(function,"quad"))type=FJ::XQUADRAT;
		else if (bitmap & FJ::Plugin)type=FJ::DLL_FUNCTION; 
		else {
		ERROR("");
printf("Incorrect function name `%s'\nif you want to use your own function use --plugin option\n",function);
		info(FJ::Hint);
		return -1;}
	}
...
\n	    [[-d PLUGIN_URL] | [--plugin PLUGIN_URL]]\
...
\n			Specify function name to be interpolated. it is usefull If you\
\n			don't want interpolation have been built from input file.\
\n			Al-Jabr has four inner functions that can be specified by this\
\n			option or you can give your own function name as well. By default\
\n			it is quad (parabola: x0^2 - 2*x0 + 1). If you want your own\
\n			function be interpolated use also -d/--plugin option.\
\n			Conflicts with -f option\
\n\n	-d,--plugin dynamically_loadable_library_url\
\n			DLL plugin name. If this option is given function will be used\
\n			from plugin. To specify function name in the plugin use\
\n			-d/--plugin option.\
\n			Conflicts with -f option\
\n\n	-f filename\
